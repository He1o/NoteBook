...
#             dist = 0
#             px = np.array(p.cars[car].pos)
#             for pidx in seq:
#                 py = np.array(p.psgs[pidx].pos)
...
#         xlis = [p.cars[car].pos[0]]
#         ylis = [p.cars[car].pos[1]]        
#         # xlis = []
#         # ylis = []
#         for ps in bestseq:
#             xlis.append(p.psgs[ps].pos[0])
#             ylis.append(p.psgs[ps].pos[1])

#         plt.plot(xlis, ylis)
    
#     plt.scatter([p.pos[0] for p in p.psgs], [p.pos[1] for p in p.psgs], c = 'blue')
#     plt.scatter([p.pos[0] for p in p.cars], [p.pos[1] for p in p.cars], c = 'red')
#     for i, psg in enumerate(p.psgs):
#         plt.text(p.psgs[i].pos[0] + 0.0001, p.psgs[i].pos[1] + 0.0001, psg.id_+','+str(psg.num))
#     for i, car in enumerate(p.cars):
#         plt.text(p.cars[i].pos[0] + 0.0001, p.cars[i].pos[1] + 0.0001, car.id_+','+str(car.sites))
...
#         # py = []
#         for ps in psg:
#             px.append(p.psgs[ps].pos[0])
#             px.append(p.cars[car].pos[0])
#             py.append(p.psgs[ps].pos[1])
#             py.append(p.cars[car].pos[1])
